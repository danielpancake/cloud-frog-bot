...
Все загрузки сохраняются в корень Яндекс.Диска под общим префиксом `/CloudFrog/<chat_id>/`, где `chat_id` -- это идентификатор чата с ботом. Внутри этой папки используется та же категоризация, что и в Telegram: `photos/`, `videos/`, `voice/`, `stickers/` и т.д.
